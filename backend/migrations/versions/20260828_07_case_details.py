...
def upgrade() -> None:
    op.add_column("analysis_cases", sa.Column("price_eur", sa.Numeric(precision=14, scale=2)))
    op.add_column("analysis_cases", sa.Column("surface_m2", sa.Numeric(precision=10, scale=2)))
...
def downgrade() -> None:
    op.drop_constraint("ck_analysis_cases_lot_count_positive", "analysis_cases", type_="check")
    op.drop_constraint("ck_analysis_cases_surface_positive", "analysis_cases", type_="check")
    op.drop_constraint("ck_analysis_cases_price_positive", "analysis_cases", type_="check")
